 ...
 */
public class MouseLevel2 extends Actor
{
    private int lives = 3;
    ...
    GreenfootSound backgroundMusic = new GreenfootSound("Alien27.mp3");
    ...
    private boolean platformInteraction() {
        if (getOneObjectAtOffset(0, 48, Platform.class) != null  && Greenfoot.isKeyDown("space") == false) {
            gravity = 0;
            jumpCount = 0;
            move(-1);
            return true;
    ...
    private void eatBread() {
        if (getOneIntersectingObject(Bread.class) != null) {
           ...
           lives--;
           breadHurt.play();
           ((Level2)getWorld()).getLives().lives();
       }
    }
    private void eatCheese() {
        if (getOneIntersectingObject(Cheese.class) != null) {
            ...
            collectSound.play();
            ((Level2)getWorld()).getCounter2().addScore();
            cheeseEaten++;
        }
    }
    private void fallOff() {
        if (getY() > 575) {
            ...
            getWorld().addObject(new Platform(), 425, 100);
